...
void Highlighter::highlightBlock(const QString &text)
{
    if (!ConfigStruct::GetConfig()->isSimpleTextMode) {
        QVector<ConfigStruct::HighlightRule> highlightRuleList = ConfigStruct::GetConfig()->highlightRuleList;
        foreach (const ConfigStruct::HighlightRule &rule, highlightRuleList){
        ...
        if (1 != previousBlockState())
            startIndex = ConfigStruct::GetConfig()->multipleLinesCommentRegExpStart.indexIn(text);
        while (0 <= startIndex){
            int endIndex = ConfigStruct::GetConfig()->multipleLinesCommentRegExpEnd.indexIn(text, startIndex);
            int commentLength;
            if (-1 == endIndex){
                ...
                commentLength = endIndex - startIndex + ConfigStruct::GetConfig()->multipleLinesCommentRegExpEnd.matchedLength();
            }
            
            setFormat(startIndex, commentLength, ConfigStruct::GetConfig()->multiLineCommentFormat);
            startIndex = ConfigStruct::GetConfig()->multipleLinesCommentRegExpStart.indexIn(text, startIndex + commentLength);
        }
    }
}
...
TextEdit::TextEdit()
{
    ...
    setFrameStyle(0);
    //setViewportMargins(ConfigStruct::GetConfig()->lineNumberAreaWide, 0, 0, 0); //行号区域大小
    ...
    connect(this, SIGNAL(cursorPositionChanged()), this, SLOT(HighlightCurrenLine()));
...
void TextEdit::SetTextEdit()
{
    ...
    pl.setBrush(QPalette::Text, QBrush(ConfigStruct::GetConfig()->textColor));  //文本颜色
    pl.setBrush(QPalette::Base, QBrush(ConfigStruct::GetConfig()->textEditBackgroundColor));    //背景颜色
    setTabStopWidth(ConfigStruct::GetConfig()->tabWide * 10);   //Tab宽度
    setPalette(pl);
...
void TextEdit::Comment()
{
    ...
    if (tCursor.hasSelection()){
        ...
        if (tCursor.atBlockStart()){
            ...
            tCursor.movePosition(QTextCursor::Right, QTextCursor::KeepAnchor, selectionCount);
            endLine = tCursor.blockNumber();
            blockCount = endLine - beginLine + 1;

            if (tCursor.atBlockEnd()){
                tCursor.movePosition(QTextCursor::Left, QTextCursor::KeepAnchor, selectionCount);
                
                for (int i = 0; i != blockCount; ++i){
                    selectionTexts.append(textBlock.text());
                    textBlock = textBlock.next();
                    if (blockCount != (i + 1))
                        selectionTexts.append("\r//");
                }
...
void TextEdit::Property()
{
    ...
    if (!fileInfo.exists())
        return;
    QMessageBox::information(this, tr("%1文件属性-iEditor").arg(fileInfo.fileName()), tr("<font color=darkblue><p>文件名称: %1</p><p>扩展名称: %2</p><p>文件大小: %3 Byte</p><p>存储位置: %4</p><p>是否可读: %5</p><p>是否可写: %6</p><p>创建时间: %7</p><p>最后访问: %8</p><p>修改时间: %9</p></font>").arg(fileInfo.fileName().split(".").first()).arg(fileInfo.suffix()).arg(fileInfo.size()).arg(fileInfo.path()).arg(fileInfo.isReadable()).arg(fileInfo.isWritable()).arg(fileInfo.created().toString()).arg(fileInfo.lastRead().toString()).arg(fileInfo.lastModified().toString()));
}
...
bool TextEdit::ReadDocument(const QString &filePath)
{
    ...
    QMessageBox::warning(this, tr("读取出错-iEditor"), tr("<p>无法读取[%1],出错信息：</p><font color=red>%2</font>").arg(filePath).arg(file.errorString()));
    file.close();
    return false;
}
...
bool TextEdit::WriteDocument(const QString &filePath)
{
    ...
    {
        QMessageBox::warning(this, tr("iEditor提示"), tr("保存[%1]文件出错0.0<p>出错信息:%2</p>").arg(filePath).arg(file.errorString()));
        return false;
    }
...
void TextEdit::HighlightCurrenLine()
{
    ...
    if (!isReadOnly()) {
        ...
        if (ConfigStruct::GetConfig()->isHighlightCurrentLine) {
            extraSelection.format.setBackground(ConfigStruct::GetConfig()->currentLineColor);
        } else {
            extraSelection.format.setBackground(ConfigStruct::GetConfig()->textEditBackgroundColor);
        }
...
void TextEdit::resizeEvent(QResizeEvent *event)
{
    ...
    lineNumberArea->setGeometry(QRect(rect.left(), rect.top(), ConfigStruct::GetConfig()->lineNumberAreaWide, rect.height()));
...
void TextEdit::DrawLinwNumberEvent(QPaintEvent *event)
{
    if (ConfigStruct::GetConfig()->isShowRowNumber){
        ...
        painter.fillRect(event->rect(), ConfigStruct::GetConfig()->lineAreaBackgroundColor);
        ...
        while (textBlock.isValid() && top <= event->rect().bottom()){
            if (textBlock.isVisible() && bottom >= event->rect().top()){
                ...
                painter.drawText(0, top, lineNumberArea->width() - lineNumberArea->width()/8, fontMetrics().height(), Qt::AlignRight | Qt::AlignVCenter, number);
            }
        ...
        QPainter painter(lineNumberArea);
        painter.fillRect(lineNumberArea->rect(), ConfigStruct::GetConfig()->lineAreaBackgroundColor);
    }
}
